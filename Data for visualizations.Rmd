...
fspider4 <- ConflictsData %>% 
  ...
  summarise(`Number of fatalities` = sum(fatalities),
            `Number of conflicts` = sum(conflicts),
            `Involving battles` = max(battles, na.rm = TRUE),
            `Involving protests` = max(protest, na.rm = TRUE),
            `Started by government` = max(government, na.rm = TRUE),
            `Started by rebel forces` = ifelse(max(rebelforce, na.rm = TRUE) == -Inf, 0, max(rebelforce, na.rm = TRUE)))
...
spider4 <- fspider4 %>% filter(country %in% c("Afghanistan", "Syria", "United States", "India", "Ukraine", 
                                              "Israel", "Palestine", "Mexico", "Nigeria", "Somalia", "Azerbaijan")) %>% 
  left_join(Indicators %>%  select(Country.Code, PV.EST) %>% group_by(Country.Code) %>% 
              summarise(PV.EST = mean(PV.EST, na.rm = TRUE)) %>% 
              mutate(`Political Stability and Absence of Violence/Terrorism` = rank(PV.EST)), 
            by = c("iso3"="Country.Code")) %>% select(-iso3, -PV.EST) %>% 
  reshape2::melt() %>% 
  group_by(variable) %>% 
  mutate(value = value / sum(value)) %>%
