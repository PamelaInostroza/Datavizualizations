...
ConflictsData %>% 
  mutate(Motiv = case_when(str_detect(notes, "coronavirus") ~ "Covid",
                           str_detect(notes, "politic") ~ "Political",
                           str(detect(notes, "government") ~ "Government")
                           TRUE ~ "Other")) %>% 
  group_by(Motiv) %>% 
  count()
...
Ind <- c("Conflicts", "Fatalities", "IT.NET.USER.ZS", "IT.CEL.SETS", "ST.INT.ARVL","ST.INT.DPRT", "SH.DYN.MORT", "SP.POP.GROW")
ParallelCoordinatesVis <- SummaryConflicts  %>% 
  mutate(Fatalities = Fatalities/ SP.POP.TOTL*100*1000,
         Conflicts = Conflicts / sum(Conflicts, na.rm = TRUE) *100 *10) %>% 
  select(year, continent, region2, all_of(Ind)) %>% 
  ...
  rename(region = region2,
         `Conflicts (% of world total per 10)` = Conflicts,
         `Fatalities (% of population per 1000)` = Fatalities,
         `Individuals using the Internet (% of population)` = IT.NET.USER.ZS, 
  `Mobile cellular subscriptions (per 10 million)` = IT.CEL.SETS, 
  `International tourism, number of arrivals (per 10 million)` = ST.INT.ARVL,
  `International tourism, number of departures (per 10 million)` = ST.INT.DPRT, 
  `Mortality rate, under-5 (per 1,000 live births)` = SH.DYN.MORT, 
  `Population growth (annual %)` = SP.POP.GROW
  )

#write.csv(ParallelCoordinatesVis, file = "Data/forvis/ParallelCoordinates2.csv", na = "")

ParallelCoordinatesVis <-  SummaryConflicts %>% 
  mutate(Fatalities = Fatalities/ SP.POP.TOTL*100*1000,
         Conflicts = Conflicts / sum(Conflicts, na.rm = TRUE) *100 *10) %>% 
  select(year, continent, region2, country, Conflicts, Fatalities,
         CC.EST, GE.EST, PV.EST, RL.EST, RQ.EST, VA.EST
         ) %>% 
  group_by(year, continent, region2) %>% 
  summarise(Conflicts = sum(Conflicts), 
  ...
  rename(region = region2,
         ...
         `Political Stability and Absence of Violence/Terrorism` = PV.EST, 
         `Rule of Law` = RL.EST, 
         `Regulatory Quality` = RQ.EST, 
         `Voice and Accountability` = VA.EST)

#write.csv(ParallelCoordinatesVis, file = "Data/forvis/ParallelCoordinates1.csv", na = "")
...
validate(Dsunburst1)
write(prettify(Dsunburst1), file = "Data/forvis/Sunburst1.json")
...
validate(Dsunburst2)
write(prettify(Dsunburst2), file = "Data/forvis/Sunburst2.json")
...
ToJsonData3 <- ConflictsData %>% 
  select(year, sub_event_type, continent, region2) %>% 
  group_by(year, sub_event_type, continent, region2) %>% 
...
zoom1 <- ToJsonData3 %>% 
  ...
  rename(name = region2) %>% 
  group_by(year, sub_event_type, continent) %>%
  tidyr::nest() %>%
  ...
  tidyr::nest() %>%
  rename(name = sub_event_type,
         children = data) %>% filter(year == 2020) %>% ungroup() %>% select(-year) %>% 
  mutate(conf = "Conflicts") %>% 
  group_by(conf) %>% 
  tidyr::nest() %>% 
  rename(name = conf, children = data)
